...
const ExitPage = () => {
  ...
  useEffect(() => {
    if (selectedSeats.length > 0) {
      toast.success(
        "You have limited 10seconds to go back and change Seating!",
        {
          autoClose: 3000,
        }
      );
  ...
  return (
    <div className="d-flex justify-content-center align-items-center vh-100 ">
      <div className="card text-center " style={{ width: "600px" }}>
        <div>
          <Link to="/" className="btn btn-primary mt-3" onC lick={handleExit}>
            Go to Home Page to start again
          </Link>
        </div>

        <div className="card-body">
          <h2 className="card-title">CheckOut Page</h2>
          <div className="card">
          ...
          <div>
            <div>
              {active === true ? (
                <Link to="/booking" className="btn btn-secondary mt-3 ml-2">
